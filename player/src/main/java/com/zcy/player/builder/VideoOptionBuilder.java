package com.zcy.player.builder;

import android.graphics.drawable.Drawable;
import android.view.View;

import com.zcy.player.listener.StateUiListener;
...
import com.zcy.player.render.effect.NoEffect;
...
public class VideoOptionBuilder {
     ...
     * 状态监听
     */
    public VideoOptionBuilder setStateUiListener(StateUiListener stateUiListener) {
        this.mStateUiListener = stateUiListener;
        return this;
    }

    public void build(StandardVideoPlayer standardVideoPlayer) {
        if (mBottomShowProgressDrawable != null && mBottomShowProgressThumbDrawable != null) {
            standardVideoPlayer.setBottomShowProgressBarDrawable(mBottomShowProgressDrawable, mBottomShowProgressThumbDrawable);
        }
        ...
        build((BaseVideoPlayer) standardVideoPlayer);
    }

    public void build(BaseVideoPlayer baseVideoPlayer) {
        baseVideoPlayer.setPlayTag(mPlayTag);
        baseVideoPlayer.setPlayPosition(mPlayPosition);

        baseVideoPlayer.setThumbPlay(mThumbPlay);

        if (mThumbImageView != null) {
            baseVideoPlayer.setThumbImageView(mThumbImageView);
        }

        baseVideoPlayer.setNeedLockFull(mNeedLockFull);

        if (mLockClickListener != null) {
            baseVideoPlayer.setLockClickListener(mLockClickListener);
        }

        baseVideoPlayer.setDismissControlTime(mDismissControlTime);


        if (mSeekOnStart > 0) {
            baseVideoPlayer.setSeekOnStart(mSeekOnStart);
        }

        baseVideoPlayer.setShowFullAnimation(mShowFullAnimation);
        baseVideoPlayer.setLooping(mLooping);
        if (mVideoAllCallBack != null) {
            baseVideoPlayer.setVideoAllCallBack(mVideoAllCallBack);
        }
        if (mVideoProgressListener != null) {
            baseVideoPlayer.setVideoProgressListener(mVideoProgressListener);
        }
        if (mStateUiListener != null) {
            baseVideoPlayer.setStateUiListener(mStateUiListener);
        }
        baseVideoPlayer.setOverrideExtension(mOverrideExtension);
        baseVideoPlayer.setAutoFullWithSize(mAutoFullWithSize);
        baseVideoPlayer.setRotateViewAuto(mRotateViewAuto);
        baseVideoPlayer.setOnlyRotateLand(mIsOnlyRotateLand);
        baseVideoPlayer.setLockLand(mLockLand);
        baseVideoPlayer.setSpeed(mSpeed, mSounchTouch);
        baseVideoPlayer.setHideKey(mHideKey);
        baseVideoPlayer.setIsTouchWiget(mIsTouchWiget);
        baseVideoPlayer.setIsTouchWigetFull(mIsTouchWigetFull);
        baseVideoPlayer.setNeedShowWifiTip(mNeedShowWifiTip);
        baseVideoPlayer.setEffectFilter(mEffectFilter);
        baseVideoPlayer.setStartAfterPrepared(mStartAfterPrepared);
        baseVideoPlayer.setReleaseWhenLossAudio(mReleaseWhenLossAudio);
        baseVideoPlayer.setFullHideActionBar(mActionBar);
        baseVideoPlayer.setShowDragProgressTextOnSeekBar(isShowDragProgressTextOnSeekBar);
        baseVideoPlayer.setFullHideStatusBar(mStatusBar);
        if (mEnlargeImageRes > 0) {
            baseVideoPlayer.setEnlargeImageRes(mEnlargeImageRes);
        }
        if (mShrinkImageRes > 0) {
            baseVideoPlayer.setShrinkImageRes(mShrinkImageRes);
        }
        baseVideoPlayer.setShowPauseCover(mShowPauseCover);
        baseVideoPlayer.setSeekRatio(mSeekRatio);
        baseVideoPlayer.setRotateWithSystem(mRotateWithSystem);
        if (mSetUpLazy) {
            baseVideoPlayer.setUpLazy(mUrl, mCacheWithPlay, mCachePath, mMapHeadData, mVideoTitle);
        } else {
            baseVideoPlayer.setUp(mUrl, mCacheWithPlay, mCachePath, mMapHeadData, mVideoTitle);
        }
    }
